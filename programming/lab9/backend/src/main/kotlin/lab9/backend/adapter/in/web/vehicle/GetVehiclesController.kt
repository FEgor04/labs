...
//        binder.registerCustomEditor(GetVehiclesQuery.Filter::class.java, GetVehiclesFilterEditor())
        binder.registerCustomEditor(GetVehiclesFilterDTO::class.java, GetVehiclesFilterDTOEditor())
    }
    ...
    fun getVehicles(
        ...
        @RequestParam filter: GetVehiclesFilterDTO?,
        principal: Principal,
    ): ResponseEntity<ShowVehiclesResponse> {
        val sorting = GetVehiclesSorting(sortingColumn, ascending)
        logger.info("Handling GET /api/vehicles with pageSize = $pageSize, pageNumber=$pageNumber, sorting = $sorting, filter = $filter")
        val request =
            GetVehiclesRequest(pageSize = pageSize, pageNumber = pageNumber, sorting = sorting, filter = filter?.toQueryFilter())
