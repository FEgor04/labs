...
const VehicleTypeFilterForm = ({props, setFilter, clearFilter, column}: FilterProps) => {
    ...
    const onFinish = (values: any) => {
        ...
        setFilter(new EnumFilter<VehicleType>(
            column,
            values.values.map((it) => {
                return VehicleType[`${it}`]
            }),
            "VehicleType"
        ))
        props.confirm({
    ...
    const options = Object.keys(VehicleType).filter((item) => {
        return isNaN(Number(item));
    }).map((it, ind) => {
        return {
            label: t(`vehicle.vehicleType.${it}`),
            value: it
        }
    });
...
export default VehicleTypeFilterForm