package lab8.client.presentation.cli.utils
...
object ReaderUtils {
    ...
    fun readVehicle(writer: BufferedWriter, reader: BufferedReader): Vehicle {
        ...
        val x: Int = readType(
            ...
            caster = toIntCaster,
            validator = { it > Coordinates.xLowerBound }
        )
        val y: Long? =
            readType(reader, writer, hint = "Введите переменную Y\n", caster = toLongOrNullCaster, validator = { true })
        val enginePower: Double = readType(
            reader,
            writer,
            hint = "Введите мощность двигателя > 0\n",
            caster = toDoubleCaster,
            validator = { it > Vehicle.enginePowerLowerBound }
        )
        val vehicleType: VehicleType? = readType(
            reader,
            writer,
        ...
        val fuelType: FuelType = readType(
            reader,
            writer,
