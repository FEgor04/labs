...
equations = [
    (
        lambda x: x**3 + 3 * x**2 + 3 * x + 1,
        "x^3 + 3x^2 + 3x + 1",
        lambda x: 3 * x**2 + 6 * x + 3,
    ),
...
def newton_method(f, fp, a, b, eps):
    ...
    for x in np.linspace(a, b, 1000):
        if fp(x) == 0:
            print(
                "Производная принимает нулевое значение. Метод Ньютона нельзя применять."
            )
            exit()
        dx = 0.00001
        ...
        if second_der_sign * prev_second_der_sign < 0:
            print(
                "Вторая производная меняет знак на отрезке. Метод Ньютона нельзя применять"
            )
            exit()
